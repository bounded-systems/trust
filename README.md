...
| Semantic git: AST-level structural merge | 🟡 Partial | Real structural 3-way **merge driver for JSON** — different-key edits merge where a text merge conflicts. Backed **both ways**: *executable* (verified against real `git merge`, PR #28) and *proven* (a Mathlib-free **Lean** proof of merge soundness, CI-gated sorry-free, PR #30). Structural diff, Rust-language merge, and node identity remain. |
