...
| Semantic git: AST-level structural merge | 🟡 Partial | Real structural 3-way **merge driver for JSON** — different-key edits merge where a text merge conflicts; verified against real `git merge` (PR #28). Rust-backed; a Lean soundness proof is the immediate follow-up. Structural diff, Rust-language merge, and node identity remain. |
