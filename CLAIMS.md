...
| 4.8 | A credential mint records **which human authorized it**. | 🟡 Partial | `infra/cloudflare/broker/src/index.mjs · mintAuditRecord`; `infra/.github/workflows/audit-verify.yml` · approver-resolution step | **The narrower true claim:** an environment-gated mint proves *that* a human approved and attributes the run to its **triggerer** — it does not attribute the **approval**. `mintAuditRecord`'s `actor` is GitHub's OIDC claim for whoever triggered the run; on a dispatch-then-approve those are routinely two different people. The token cannot close this: GitHub signs `environment` (proof approval occurred) but the Actions claim set has **no approver claim**, so approver identity exists only in the Actions REST API. Resolved **out of band** — `audit-verify.yml` joins mint records to `GET /actions/runs/{id}/approvals` and surfaces approver plus a self-approval warning (approver == triggerer ⇒ the gate admitted no second party). Two honest limits: warning-only (it must not redden a deploy), and same-repo only (cross-repo mints are reported *unresolved*, never silently skipped). The stronger in-broker fetch is **deliberately rejected** — it would put an unsigned network input inside a boundary whose virtue is verifying exactly one signed thing (`infra/cloudflare/broker/SECURITY.md`, actor-provenance section, 2026-08-11). |
...
| 8.1b | AST-level structural 3-way **merge**. | ✅ Enforced | `git-ast` has a real structural 3-way merge for JSON (`git-ast/src/merge.rs`, wired into `git merge` by `git-ast setup`): edits/additions to *different* object keys merge cleanly where a text merge would conflict; same-key divergence conflicts; fail-closed (standard conflict markers) otherwise. **Backed both ways** — *executable*: verified against real `git merge` by the cucumber claims suite + CI (PR #28); *proven*: a Mathlib-free **Lean** proof (`git-ast/proofs/JsonMerge.lean`, CI-gated sorry-free) proves idempotence + only-one-side and `decide`s the shared vectors (`tests/merge_vectors.json`) that the Rust suite also runs (PR #30). | The merge *mechanism* is enforced (run **and** proven). Coverage is additive — same standard as 8.1a: JSON today; Rust-language merge and array element merging widen it. Lean follow-ups: symmetry + deep preservation (need canonical key ordering, 8.1a) and a recursive nested model. |
| 8.1c | AST structural **diff** driver. | ✅ Enforced | `git-ast`'s diff driver (`git-ast/src/diff.rs`, wired into `git diff` by `git-ast setup`) reports JSON changes as object-key paths added/removed/changed (`~ a.b: 1 -> 2`), order-independent — *what* changed semantically, not which lines moved. Verified against real `git diff` by the cucumber claims suite + CI (PR #31); non-JSON falls back to a text diff. | Coverage additive (same standard as 8.1a/8.1b): JSON today; Rust-language and array element-level diff widen it. "Cleaner diffs via canonical formatting" is separately delivered under 8.1a — this is the *structural* diff layer above it. |
| 8.1d | Stable AST **node identity** (move/rename tracking; refactor-aware history). | 🟡 Partial | `git-ast match <old> <new>` (`git-ast/src/identity.rs`) corresponds top-level Rust functions across versions. **Exact**, content-addressed matches — a function tracked through a **rename** (`shape_hash` = content with the declared name blanked), a **reorder** (position-independent), or a **body edit**, plus add/remove (PR #32). Plus a **structural fuzzy** pass — a function **renamed *and* edited at once** (even with statements reordered) is recovered by Sørensen–Dice over **Merkle subtree hashes** (`git-ast/src/printer.rs::subtree_hashes` — GumTree's bottom-up phase; per-node deep/Merkle content), instead of a spurious add/remove (PRs #33, #34). And `git-ast match --script` prints a statement-level **edit script** inside each changed function — `moved`/`changed`/`inserted`/`deleted` with **move** detection (the GumTree top-down phase at statement granularity; `git-ast/src/identity.rs::edit_script`, PR #35). Real-CLI-verified by CI. | Toward ✅, the deeper research remains: **sub-statement** edit scripts (recurse into a changed expression tree), the remaining identity axes (binding identity via name resolution, use-site identity, deep/Merkle content), non-`fn` items, cross-file matching, and persistence (git notes). See the node-identity essay in `git-ast/README.md`. |
