...
| # | Claim | Grade | Evidence | Notes |
|---|---|---|---|---|
| 3.1 | Each domain has exactly **one** sanctioned access point (subprocess, env, host, fs, secrets, git, gh…), enforced by tests — no upward deps, no ambient authority. | ✅ Enforced | 22 × `extractability.test.ts` (e.g. `cas`, `proc`, `env`, `host`, `fs`, `auth`, `git`, `policy`) | Zero-tolerance: tests assert `violations === []` for `child_process`, `process.env`, raw `node:fs`, etc. outside the allowlist. |
| 3.2 | Seam coverage is **complete** — nothing escapes the net. | 🔴 Gap | bead `prx-5yp`; `prx/docs/claims-audit-instrument-v0.md` | ~13 leaf files import `node:fs`/`node:os` directly, outside the fs/host seam, with no extractability test yet. Either close them or baseline them explicitly. |
...
| 4.4 | Attestations are verifiable by **off-the-shelf** in-toto/DSSE tooling. | 🟡 Partial | `anchored-chain/README.md:61` | Formats emitted; no confirmed integration with an external verifier yet. |
...
| # | Claim | Grade | Evidence | Notes |
|---|---|---|---|---|
| 6.1 | git-writes are signed **and verified against their owner**. | 🟡 Partial | `prx` provenance signing; `PRX_REQUIRE_SIGNED_DERIVATIONS` | Signature *validity* is checked; **ownership** is not, on the live path. Verification is opt-in and fail-open, git effects only. |
| 6.2 | A privileged effect not produced by its owning actor fails verification. | 🔴 Gap | `prx/.../provenance/effect-ownership.ts · verifyEffectOwnership` (defined + exercised only in `value_props.ts` instrument) | **No call site in the merge-guard / publisher path.** Tracked: bead `prx-6s8` (wire it in + default-on signed verification). |
...
- `prx-6s8` — Wire `verifyEffectOwnership` into the merge-guard/publisher path; default-on signed verification.
