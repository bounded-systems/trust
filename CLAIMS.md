...
| 6.3 | Caveat / attenuation enforcement is by interposition (proxy), so a narrowed door is a genuinely weaker capability. | 🟡 Partial | wired into launcherd ([claude-box#163](https://github.com/bounded-systems/claude-box/pull/163)); bead `prx-yweb` | Re-framed: "string compare" mischaracterized the gap — `checkCaveats` already dispatches broker verifiers fail-closed and `attenuatesDoors` is a proven set-superset (65k cases). The real gap was that narrowing is **metadata**: a delegated child holds the *same upstream reference*. Now closed in code: the rest of `prx-86g9` (reference-passing spawn #158/#159, cgroup caller-correlation #161, lineage-retirement #162) landed, and `launcherd.handleLaunch` **fronts each caveated unix door with an interposer** (`guest-room/interpose.ts`) that holds the upstream socket and runs `checkCaveats` per request — the box mounts only the proxy, so an off-caveat request never reaches upstream. Unit-tested on real sockets, **and e2e-verified across a real podman bind-mount in the Linux VM**: a *client container* reaching the interposer socket (created by a separate harness container, shared via a podman volume) gets `read` allowed but `write` (off-caveat) `DENIED:caveat not satisfied` — and the upstream's log shows the denied call **never arrived**. So the keystone holds across the actual container boundary, not just in-process. The only unrun step is the full `launcherd`-orchestrated in-pod spawn, which composes the VM-verified per-door mounts (#159/#161) with the unit-tested fronting glue. |
