...
| 6.3 | Caveat / attenuation enforcement is by interposition (proxy), so a narrowed door is a genuinely weaker capability. | 📐 Design-only | interposer prototype ([guest-room#41](https://github.com/bounded-systems/guest-room/pull/41)); bead `prx-yweb` | Re-framed: "string compare" mischaracterized the gap — `checkCaveats` already dispatches broker verifiers fail-closed and `attenuatesDoors` is a proven set-superset (65k cases). The real gap was that narrowing is **metadata**: a delegated child holds the *same upstream reference*. An **interposer** (`guest-room/interpose.ts`, [docs](https://github.com/bounded-systems/guest-room/blob/main/docs/interposition.md)) makes it structural — it holds the upstream ref, runs `checkCaveats` per request, and hands the child only the proxy ref; a denied request **never reaches upstream** (keystone test). Prototyped + tested; ✅ awaits wiring into spawn, gated on reference-passing spawn (`prx-86g9`/`prx-8k08`). |
