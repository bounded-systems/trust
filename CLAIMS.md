...
| 8.1d | Stable AST **node identity** (move/rename tracking; refactor-aware history). | 🟡 Partial | First slice built: `git-ast match <old> <new>` (`git-ast/src/identity.rs`) corresponds top-level Rust functions across versions by content-addressed hashing — tracking a function through a **rename** (`shape_hash` = content with the declared name blanked), a **reorder** (position-independent), or a **body edit**, plus add/remove. The *exact*, zero-heuristic matches (the essay's "by matching" family). Real-CLI-verified by CI (PR #32). | Toward ✅, the hard core remains: **fuzzy** matching (a node renamed *and* edited at once — GumTree-family), the deeper identity axes (binding identity via name resolution, use-site identity, deep/Merkle content), non-`fn` items, cross-file matching, and persistence (git notes). See the node-identity essay in `git-ast/README.md`. |
