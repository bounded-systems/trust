...
| 8.1d | Stable AST **node identity** (move/rename tracking; refactor-aware history). | 📐 Design-only | Tracking a node through a move/rename — the floor under refactor-aware blame and a semantic diff that *follows* nodes — is the hardest open problem (see the node-identity essay in `git-ast/README.md`). `git-ast inspect`'s formatting-invariant content hash is the only seam so far. | The genuine research frontier (GumTree-family matching, content-addressed subtrees), explicitly out of scope. |
