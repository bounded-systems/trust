...
| 8.1b | AST-level **semantic merge** (structural 3-way merge; AST diff/merge drivers). | 🟡 Partial | `git-ast` has a real structural 3-way **merge driver for JSON** (`git-ast/src/merge.rs`, wired by `git-ast setup`): on `git merge`, edits/additions to *different* object keys merge cleanly where a text merge would conflict; same-key divergence conflicts. **Backed both ways** — *executable*: verified against real `git merge` by the cucumber claims suite + CI (PR #28); *proven*: a Mathlib-free **Lean** proof of the merge's soundness (`git-ast/proofs/JsonMerge.lean`, CI-gated sorry-free) proves idempotence + only-one-side and `decide`s the shared vectors (`tests/merge_vectors.json`) that the Rust suite also runs (PR #30). | Still placeholder toward ✅: the structural **diff** driver, Rust-language merge, array element merging, and **stable node identity** (move/rename tracking). Lean follow-ups: symmetry + deep preservation (need canonical key ordering, claim 8.1a) and a recursive nested model. |
